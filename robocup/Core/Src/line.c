...
void turn(int turn_speed)
{
    if(turn_speed>0)
	{
        ...
        __HAL_TIM_SET_COMPARE(&htim4,TIM_CHANNEL_2,200+50*turn_speed);  //ina1 ina2 -> channel 2  left
	...
	else
	{
	    ...
	    HAL_GPIO_WritePin(ina2_GPIO_Port,ina2_Pin,GPIO_PIN_SET);
	    HAL_GPIO_WritePin(inb2_GPIO_Port,inb2_Pin,GPIO_PIN_RESET);
	    __HAL_TIM_SET_COMPARE(&htim4,TIM_CHANNEL_1,200+50*(-turn_speed));  //inb1 inb2 -> channel 1  right
	    HAL_Delay(10);
	}
}
